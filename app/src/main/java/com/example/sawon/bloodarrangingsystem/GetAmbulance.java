...
public class GetAmbulance extends AppCompatActivity {
    ListView listView;
    TextView heding;
    String tag_json_arry = "json_array_req";
    //String URL = "http://dalia.a0001.net/getdata.php";
    String URL = "http://192.168.0.103/MyAPI/android/bas/Api.php";
    List<Amb> ambulanceArrayList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        heding.setText("Ambulance BD");
        listView = findViewById(lv_ambulance_list);
        ambulanceArrayList=new ArrayList<Amb>();
                        ...
                        try {
                            for (int i = 0; i < response.length(); i++) {
                               JSONObject ambulancrObject = response.getJSONObject(i);
                                ...
                                Amb ambulance = new Amb(Integer.parseInt(ambulancrObject.getString("id")), ambulancrObject.getString("contact"), ambulancrObject.getString("region"), ambulancrObject.getString("service_time"));
                                ambulanceArrayList.add(ambulance);
                            }

                            Log.d("arraylist:", ambulanceArrayList.toString());
                            //creating custom adapter object
                            ListViewAdapter adapter = new ListViewAdapter(ambulanceArrayList, getApplicationContext());
